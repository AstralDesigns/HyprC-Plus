...
var Daemon = class {
    ...
    constructor(dock) {
        ...
        this.desktopPinnedApps = new Set();
    ...
    // ── Desktop pinned apps (separate file: ~/.config/desktop-pinned) ─────
    loadDesktopPinnedApps() {
        const file = Gio.File.new_for_path(`${GLib.getenv('HOME')}/.config/desktop-pinned`);
        if (file.query_exists(null)) {
            const [, contents] = file.load_contents(null);
            _decoder.decode(contents).trim().split('\n').forEach(a => {
                const cls = a.trim();
                if (cls) this.desktopPinnedApps.add(cls);
            });
        }
        console.log(`🖥️  Loaded ${this.desktopPinnedApps.size} desktop-pinned apps`);

        // Watch for external writes (e.g. from the QML DesktopLayer "Unpin"
        // action) so desktopPinnedApps stays in sync without a restart.
        if (!this._desktopPinnedMonitor) {
            try {
                this._desktopPinnedMonitor = file.monitor_file(Gio.FileMonitorFlags.NONE, null);
                this._desktopPinnedMonitor.connect('changed', (_mon, _f, _other, ev) => {
                    if (ev !== Gio.FileMonitorEvent.CHANGES_DONE_HINT &&
                        ev !== Gio.FileMonitorEvent.CREATED &&
                        ev !== Gio.FileMonitorEvent.CHANGED) return;
                    // Reload in-memory set from disk
                    this.desktopPinnedApps.clear();
                    try {
                        const f2 = Gio.File.new_for_path(`${GLib.getenv('HOME')}/.config/desktop-pinned`);
                        if (f2.query_exists(null)) {
                            const [, raw] = f2.load_contents(null);
                            _decoder.decode(raw).trim().split('\n').forEach(a => {
                                const c = a.trim(); if (c) this.desktopPinnedApps.add(c);
                            });
                        }
                    } catch (_) {}
                    console.log(`🖥️  desktop-pinned reloaded (${this.desktopPinnedApps.size} apps)`);
                });
            } catch (e) {
                console.error('[daemon] desktop-pinned monitor setup failed:', e.message);
            }
        }
    }
    ...
    saveDesktopPinnedApps() {
        const file = Gio.File.new_for_path(`${GLib.getenv('HOME')}/.config/desktop-pinned`);
        const content = _encoder.encode(Array.from(this.desktopPinnedApps).join('\n') + '\n');
        try {
            file.replace_contents(content, null, false, Gio.FileCreateFlags.REPLACE_DESTINATION, null);
            console.log(`💾 Saved ${this.desktopPinnedApps.size} desktop-pinned apps`);
            try { GLib.spawn_command_line_async('qs ipc call bar refreshDesktop'); } catch (_) {}
        } catch (e) { console.error('❌ saveDesktopPinnedApps failed:', e.message); }
    }
    ...
    shutdown() {
        ...
        if (this._desktopPinnedMonitor) {
            this._desktopPinnedMonitor.cancel();
            this._desktopPinnedMonitor = null;
        }
