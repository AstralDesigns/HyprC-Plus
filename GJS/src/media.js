...
function createMediaBox() {
    ...
    function _resolveBgColors(widget) {
        try {
            ...
            const [ok5, c5] = sc.lookup_color('source_color');
            if (ok1 && ok2) {
                _bgColors = {
		    ...
		    srcc: ok5 ? { r: c5.red, g: c5.green, b: c5.blue }
                              : { r: 1, g: 1, b: 1 },  // fallback: white
                };
            }
        } catch (e) { }
    ...
    const CAVA_RANGE     = 15;    // ascii_max_range in config
    const CAVA_N_MAX     = 64;    // bar count in config / buffer ceiling
    ...
    const CAVA_CFG_PATH  = GLib.build_filenamev([GLib.get_user_runtime_dir(), 'hyprcandy-media-cava.ini']);

    const _cavaBars  = new Float32Array(CAVA_N_MAX);
    let   _cavaN     = CAVA_N_MAX;
    let   _cavaOn    = false;
    let   _cavaProc  = null;
    ...
    cavaRingDa.set_draw_func((_w, cr, w, h) => {
        ...
        const _cp = _bgColors ? _bgColors.srcc : { r: 0.6, g: 0.85, b: 1.0 };
        const N  = _cavaN;
    ...
    function _cavaReadLine() {
        ...
        _cavaStream.read_line_async(GLib.PRIORITY_HIGH, null, (s, res) => {
            ...
            try {
                ...
                if (line === null) {
                    // EOF — cava exited; restart after delay
                    _cavaOn = false; _cavaStream = null; _cavaProc = null;
                    _scheduleRetry();
                    return;
                }
    ...
    function _startCava() {
        if (_cavaOn) return;
        // Write config then spawn cava directly — no socket manager needed.
        const cfg =
            '[general]\n' +
            'bars = ' + CAVA_N_MAX + '\n' +
            'framerate = 60\n' +
            ...
            'channels = mono\n';
        try {
            GLib.file_set_contents(CAVA_CFG_PATH, cfg);
        } catch (e) {
    ...
    function _stopCava() {
        ...
        _cavaClearRetry();
        _cavaRetryCount = 0;
        if (_cavaProc) { try { _cavaProc.force_exit(); } catch (e) {} _cavaProc = null; }
        _cavaStream = null;
        _cavaBars.fill(0);
        cavaRingDa.queue_draw();
    }
