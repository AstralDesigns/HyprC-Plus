...
case $OPEN_COUNT in
    ...
    1)
        ...
        notify-send "Candy Applets" "  Opening Utilities" -t 2000 2>/dev/null || true
        ;;
    2)
        # One open - open System Monitor
        touch "$TOGGLE_DIR/toggle-system"
        notify-send "Candy Applets" "  Opening System Monitor" -t 2000 2>/dev/null || true
        ;;
    3)
        # Three open - open Weather
        touch "$TOGGLE_DIR/toggle-weather"
        notify-send "Candy Applets" "󰖐  Opening Weather" -t 2000 2>/dev/null || true
        ;;
    4)
        # All open - show info
        notify-send "Candy Applets" "  All widgets open!\nClick to cycle or use toggle scripts." -t 3000 2>/dev/null || true
        ;;
esac
