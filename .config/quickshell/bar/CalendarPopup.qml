...
PanelWindow {
    ...
    // Matugen variable names currently written to hyprviz.conf (empty = hex/unknown)
    property string _activeBorderVar:   ""
    property string _inactiveBorderVar: ""
    // "matugen" or "hex" — which panel is shown for each picker
    property string _activeBorderMode:   "matugen"
    property string _inactiveBorderMode: "matugen"
    // Raw 6-char hex (no prefix) used when mode == "hex"
    property string _activeBorderHex:   "ffffff"
    property string _inactiveBorderHex: "000000"
    ...
    Process {
        ...
        command: ["bash", "-c",
            ...
            'active=$(grep "col.active_border" "$f" 2>/dev/null | head -1); ' +
            'echo "$active" | grep -oP "(?<=\\$)\\w+" | head -1 || echo "$active" | grep -oP "(?<=0x[fF]{2})[0-9a-fA-F]{6}" | head -1 || echo ""; ' +
            'inactive=$(grep "col.inactive_border" "$f" 2>/dev/null | head -1); ' +
            'echo "$inactive" | grep -oP "(?<=\\$)\\w+" | head -1 || echo "$inactive" | grep -oP "(?<=0x[fF]{2})[0-9a-fA-F]{6}" | head -1 || echo ""; ' +
        ...
        Component.onCompleted: {
            // Restore persisted border variable names + mode + hex on startup
            ...
            ccWin._activeBorderMode   = ccBorderColorSettings.activeBorderMode   || "matugen"
            ccWin._inactiveBorderMode = ccBorderColorSettings.inactiveBorderMode || "matugen"
            if (ccBorderColorSettings.activeBorderHex   !== "") ccWin._activeBorderHex   = ccBorderColorSettings.activeBorderHex
            if (ccBorderColorSettings.inactiveBorderHex !== "") ccWin._inactiveBorderHex = ccBorderColorSettings.inactiveBorderHex
        }
        onExited: {
            ...
            // Parse border variable names (e.g. "primary", "inverse_primary")
            // Map variable name → Theme color so the swatch stays in sync
            const varToTheme = {
                primary: Theme.cPrimary, on_primary: Theme.cOnPrimary,
                primary_container: Theme.cPrimaryContainer, on_primary_container: Theme.cOnPrimaryContainer,
                secondary: Theme.cSecondary, on_secondary: Theme.cOnSecondary,
                secondary_container: Theme.cSecondaryContainer,
                tertiary: Theme.cTertiary, tertiary_container: Theme.cTertiaryContainer,
                inverse_primary: Theme.cInversePrimary,
                background: Theme.cBackground, on_background: Theme.cOnBackground,
                surface: Theme.cSurface, on_surface: Theme.cOnSurf,
                surface_variant: Theme.cSurfVariant, on_surface_variant: Theme.cOnSurfVar,
                surface_container_low: Theme.cSurfLow, surface_container: Theme.cSurfMid,
                surface_container_high: Theme.cSurfHi, surface_container_highest: Theme.cSurfHighest,
                inverse_surface: Theme.cInverseSurface,
                outline: Theme.cOutline, outline_variant: Theme.cOutVar,
                error: Theme.cErr, error_container: Theme.cErrContainer,
                scrim: Theme.cScrim, shadow: Theme.cShadow,
                primary_fixed: Theme.cPrimaryFixed, primary_fixed_dim: Theme.cPrimaryFixedDim,
                on_primary_fixed: Theme.cOnPrimaryFixed, on_primary_fixed_variant: Theme.cOnPrimaryFixedVariant,
                source_color: Theme.cPrimary, surface_tint: Theme.cPrimary
            }
            // lines[7] = active border value: either a var name (e.g. "primary") or a 6-char hex
            if (lines.length > 7 && lines[7] && lines[7].length > 0) {
                const aVal = lines[7]
                if (varToTheme[aVal]) {
                    // It's a matugen variable name
                    ccWin._activeBorderVar   = aVal
                    ccWin._activeBorderColor = varToTheme[aVal]
                    ccWin._activeBorderMode  = "matugen"
                    ccBorderColorSettings.activeBorderMode = "matugen"
                } else if (/^[0-9a-fA-F]{6}$/.test(aVal)) {
                    // It's a raw hex color (6 chars, no prefix)
                    ccWin._activeBorderHex   = aVal
                    ccWin._activeBorderColor = "#" + aVal
                    ccWin._activeBorderVar   = ""
                    ccWin._activeBorderMode  = "hex"
                    ccBorderColorSettings.activeBorderMode = "hex"
                    ccBorderColorSettings.activeBorderHex  = aVal
                }
            }
            // lines[8] = inactive border value
            if (lines.length > 8 && lines[8] && lines[8].length > 0) {
                const iVal = lines[8]
                if (varToTheme[iVal]) {
                    ccWin._inactiveBorderVar   = iVal
                    ccWin._inactiveBorderColor = varToTheme[iVal]
                    ccWin._inactiveBorderMode  = "matugen"
                    ccBorderColorSettings.inactiveBorderMode = "matugen"
                } else if (/^[0-9a-fA-F]{6}$/.test(iVal)) {
                    ccWin._inactiveBorderHex   = iVal
                    ccWin._inactiveBorderColor = "#" + iVal
                    ccWin._inactiveBorderVar   = ""
                    ccWin._inactiveBorderMode  = "hex"
                    ccBorderColorSettings.inactiveBorderMode = "hex"
                    ccBorderColorSettings.inactiveBorderHex  = iVal
                }
            }
    ...
    Settings {
        ...
        category: "cc-border-colors-v3"
        property string activeBorderColor:   ""
        property string inactiveBorderColor: ""
        property string activeBorderVar:    ""
        property string inactiveBorderVar:  ""
        property string activeBorderMode:   "matugen"
        property string inactiveBorderMode: "matugen"
        property string activeBorderHex:    "ffffff"
        property string inactiveBorderHex:  "000000"
    }
                ...
                StackLayout {
                    ...
                    CCScrollPane {
                        ColumnLayout {
                            ...
                            property var _borderVarModel: [
                                { label: "primary",                varName: "primary",                color: Theme.cPrimary },
                                { label: "on_primary",             varName: "on_primary",             color: Theme.cOnPrimary },
                                { label: "primary_container",      varName: "primary_container",      color: Theme.cPrimaryContainer },
                                { label: "inverse_primary",        varName: "inverse_primary",        color: Theme.cInversePrimary },
                                { label: "primary_fixed",          varName: "primary_fixed",          color: Theme.cPrimaryFixed },
                                { label: "primary_fixed_dim",      varName: "primary_fixed_dim",      color: Theme.cPrimaryFixedDim },
                                { label: "secondary",              varName: "secondary",              color: Theme.cSecondary },
                                { label: "secondary_container",    varName: "secondary_container",    color: Theme.cSecondaryContainer },
                                { label: "tertiary",               varName: "tertiary",               color: Theme.cTertiary },
                                { label: "tertiary_container",     varName: "tertiary_container",     color: Theme.cTertiaryContainer },
                                { label: "background",             varName: "background",             color: Theme.cBackground },
                                { label: "surface",                varName: "surface",                color: Theme.cSurface },
                                { label: "surface_variant",        varName: "surface_variant",        color: Theme.cSurfVariant },
                                { label: "surface_container",      varName: "surface_container",      color: Theme.cSurfMid },
                                { label: "surface_container_high", varName: "surface_container_high", color: Theme.cSurfHi },
                                { label: "outline",                varName: "outline",                color: Theme.cOutline },
                                { label: "outline_variant",        varName: "outline_variant",        color: Theme.cOutVar },
                                { label: "error",                  varName: "error",                  color: Theme.cErr },
                                { label: "scrim",                  varName: "scrim",                  color: Theme.cScrim },
                                { label: "source_color",           varName: "source_color",           color: Theme.cPrimary }
                            ]

                            // ── Helper: write value to hyprviz.conf and reload ─────
                            // value is either "$varname" or "0xffRRGGBB"
                            function _writeBorderColor(field, value) {
                                _borderColorWrite.command = ["bash", "-c",
                                    'f="$HOME/.config/hypr/hyprviz.conf"; ' +
                                    'sed -i "s|^\\\\(\\\\s*\\\\)col\\\\.' + field + '\\\\s*=.*|\\\\1col.' + field + ' = ' + value + '|" "$f"; ' +
                            ...
                            RowLayout {
                                ...
                                Text {
                                    text: "Active Border"
                                    color: Theme.cPrimary
                                    font.family: Config.labelFont; font.pixelSize: 13; font.weight: 600
                                    Layout.fillWidth: true
                                }
                                // Matugen / Hex segmented toggle
                                Rectangle {
                                    implicitWidth: 140; implicitHeight: 26; radius: 8
                                    color: Qt.rgba(Theme.cInversePrimary.r, Theme.cInversePrimary.g, Theme.cInversePrimary.b, 0.12)
                                    border.width: 1
                                    border.color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.18)
                                    Row {
                                        anchors { fill: parent; margins: 2 }; spacing: 2
                                        Repeater {
                                            model: ["matugen", "hex"]
                                            delegate: Rectangle {
                                                required property string modelData
                                                property bool _on: ccWin._activeBorderMode === modelData
                                                width: (parent.width - 2) / 2; height: parent.height; radius: 6
                                                color: _on ? Qt.rgba(Theme.cInversePrimary.r, Theme.cInversePrimary.g, Theme.cInversePrimary.b, 0.82) : "transparent"
                            ...
                            // ── Status row (swatch + current value) ───────────────
                            RowLayout {
                                Layout.fillWidth: true
                                Rectangle {
                                    width: 20; height: 14; radius: 4
                                    color: ccWin._activeBorderColor
                                    border.width: 1; border.color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.35)
                                }
                                Text {
                                    text: ccWin._activeBorderMode === "matugen"
                                        ? (ccWin._activeBorderVar !== "" ? "$" + ccWin._activeBorderVar : "— not set —")
                                        : "#" + ccWin._activeBorderHex.toUpperCase()
                            ...
                            // ── Matugen variable chip grid ─────────────────────────
                            Flow {
                                Layout.fillWidth: true; spacing: 5
                                visible: ccWin._activeBorderMode === "matugen"
                                Repeater {
                                    model: _borderVarModel
                                    delegate: Rectangle {
                                        required property var modelData
                                        width: 22; height: 22; radius: 5
                                        color: modelData.color
                                        border.width: ccWin._activeBorderVar === modelData.varName ? 2 : 0
                                        border.color: Theme.cPrimary
                                        MouseArea {
                                            anchors.fill: parent; cursorShape: Qt.PointingHandCursor
                                            onClicked: {
                                                ccWin._activeBorderColor = modelData.color
                                                ccWin._activeBorderVar   = modelData.varName
                                                ccBorderColorSettings.activeBorderVar = modelData.varName
                                                _writeBorderColor("active_border", "$" + modelData.varName)
                                            }
                                        }
                                        ToolTip.visible: _aTT.containsMouse
                                        ToolTip.text: "$" + modelData.varName
                                        HoverHandler { id: _aTT }
                                    }
                                }
                            }
                            ...
                            ColumnLayout {
                                Layout.fillWidth: true; spacing: 6
                                visible: ccWin._activeBorderMode === "hex"

                                // Text entry for manual hex
                                RowLayout {
                                    Layout.fillWidth: true; spacing: 6
                                    Text {
                                        text: "#"
                                    ...
                                    Rectangle {
                                        ...
                                        border.width: 1
                                        border.color: _aHexInput.activeFocus
                                        ...
                                        TextInput {
                                            ...
                                            text: ccWin._activeBorderHex.toUpperCase()
                                            color: Theme.cPrimary
                                            font.family: Config.labelFont; font.pixelSize: 12
                                            font.capitalization: Font.AllUppercase
                                            verticalAlignment: TextInput.AlignVCenter
                                            maximumLength: 6; clip: true
                                            validator: RegularExpressionValidator { regularExpression: /^[0-9a-fA-F]{0,6}$/ }
                                            onEditingFinished: {
                                                if (text.length === 6) {
                                                    ccWin._activeBorderHex   = text.toLowerCase()
                                                    ccWin._activeBorderColor = "#" + text
                                                    ccWin._activeBorderVar   = ""
                                                    ccBorderColorSettings.activeBorderHex = text.toLowerCase()
                                                    _writeBorderColor("active_border", "0xff" + text.toLowerCase())
                                                }
                                            }
                                        }
                                    }
                                    ...
                                    Rectangle {
                                        width: 28; height: 28; radius: 7
                                        color: _aHexInput.text.length === 6 ? "#" + _aHexInput.text : ccWin._activeBorderColor
                                        border.width: 1; border.color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.3)
                                    }
                                    Text {
                                        text: "Enter & apply"
                                ...
                                // Quick-pick from palette swatches
                                Text {
                                    text: "Quick pick"
                                    color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.45)
                                    font.family: Config.labelFont; font.pixelSize: 10
                                ...
                                Flow {
                                    ...
                                    Repeater {
                                        ...
                                        delegate: Rectangle {
                                            ...
                                            width: 22; height: 22; radius: 5
                                            color: modelData.color
                                            property string _hex: modelData.color.toString().replace("#", "").substring(0, 6)
                                            border.width: ccWin._activeBorderHex.toLowerCase() === _hex.toLowerCase() ? 2 : 0
                                            border.color: Theme.cPrimary
                                            MouseArea {
                                                anchors.fill: parent; cursorShape: Qt.PointingHandCursor
                                                onClicked: {
                                                    const h = parent._hex.toLowerCase()
                                                    ccWin._activeBorderColor = modelData.color
                                                    ccWin._activeBorderHex   = h
                                                    ccWin._activeBorderVar   = ""
                                                    _aHexInput.text = h.toUpperCase()
                                                    ccBorderColorSettings.activeBorderHex = h
                                                    _writeBorderColor("active_border", "0xff" + h)
                                                }
                                            }
                                            ToolTip.visible: _aqTT.containsMouse
                                            ToolTip.text: "#" + _hex.toUpperCase()
                                            HoverHandler { id: _aqTT }
                                        }
                                    }
                                }
                            }
                                ...
                                Rectangle {
                                    ...
                                    Row {
                                        ...
                                        Repeater {
                                            ...
                                            delegate: Rectangle {
                                                required property string modelData
                                                property bool _on: ccWin._inactiveBorderMode === modelData
                                                width: (parent.width - 2) / 2; height: parent.height; radius: 6
                                                color: _on ? Qt.rgba(Theme.cInversePrimary.r, Theme.cInversePrimary.g, Theme.cInversePrimary.b, 0.82) : "transparent"
                                                border.width: _on ? 1 : 0
                                                border.color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.45)
                            ...
                            RowLayout {
                                ...
                                Rectangle {
                                    width: 20; height: 14; radius: 4
                                    color: ccWin._inactiveBorderColor
                                    border.width: 1; border.color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.35)
                                }
                                Text {
                                    text: ccWin._inactiveBorderMode === "matugen"
                                        ? (ccWin._inactiveBorderVar !== "" ? "$" + ccWin._inactiveBorderVar : "— not set —")
                                        : "#" + ccWin._inactiveBorderHex.toUpperCase()
                                    color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.6)
                                    font.family: Config.labelFont; font.pixelSize: 10
                                    elide: Text.ElideRight; Layout.fillWidth: true
                                }
                            ...
                            Flow {
                                Layout.fillWidth: true; spacing: 5
                                visible: ccWin._inactiveBorderMode === "matugen"
                                Repeater {
                                    model: _borderVarModel
                                    delegate: Rectangle {
                                        required property var modelData
                                        width: 22; height: 22; radius: 5
                                        color: modelData.color
                                        border.width: ccWin._inactiveBorderVar === modelData.varName ? 2 : 0
                                        border.color: Theme.cPrimary
                                        MouseArea {
                                            anchors.fill: parent; cursorShape: Qt.PointingHandCursor
                                            onClicked: {
                                                ccWin._inactiveBorderColor = modelData.color
                                                ccWin._inactiveBorderVar   = modelData.varName
                                                ccBorderColorSettings.inactiveBorderVar = modelData.varName
                                                _writeBorderColor("inactive_border", "$" + modelData.varName)
                                            }
                                        }
                                        ToolTip.visible: _iTT.containsMouse
                                        ToolTip.text: "$" + modelData.varName
                                        HoverHandler { id: _iTT }
                                    }
                                }
                            }
                            ...
                            ColumnLayout {
                                Layout.fillWidth: true; spacing: 6
                                visible: ccWin._inactiveBorderMode === "hex"
                                ...
                                RowLayout {
                                    ...
                                    Rectangle {
                                        ...
                                        color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.06)
                                        border.width: 1
                                        border.color: _iHexInput.activeFocus
                                            ? Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.6)
                                            : Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.2)
                                        Behavior on border.color { ColorAnimation { duration: 120 } }
                                        TextInput {
                                            id: _iHexInput
                                            anchors { fill: parent; margins: 7 }
                                            text: ccWin._inactiveBorderHex.toUpperCase()
                                            color: Theme.cPrimary
                                            font.family: Config.labelFont; font.pixelSize: 12
                                            font.capitalization: Font.AllUppercase
                                            verticalAlignment: TextInput.AlignVCenter
                                            ...
                                            onEditingFinished: {
                                                if (text.length === 6) {
                                                    ccWin._inactiveBorderHex   = text.toLowerCase()
                                                    ccWin._inactiveBorderColor = "#" + text
                                                    ccWin._inactiveBorderVar   = ""
                                                    ccBorderColorSettings.inactiveBorderHex = text.toLowerCase()
                                                    _writeBorderColor("inactive_border", "0xff" + text.toLowerCase())
                                                }
                                            }
                                        }
                                    }
                                    Rectangle {
                                        width: 28; height: 28; radius: 7
                                        color: _iHexInput.text.length === 6 ? "#" + _iHexInput.text : ccWin._inactiveBorderColor
                                        border.width: 1; border.color: Qt.rgba(Theme.cPrimary.r, Theme.cPrimary.g, Theme.cPrimary.b, 0.3)
                                    }
                                    Text {
                                        text: "Enter & apply"
                                ...
                                Flow {
                                    ...
                                    Repeater {
                                        ...
                                        delegate: Rectangle {
                                            ...
                                            color: modelData.color
                                            property string _hex: modelData.color.toString().replace("#", "").substring(0, 6)
                                            border.width: ccWin._inactiveBorderHex.toLowerCase() === _hex.toLowerCase() ? 2 : 0
                                            border.color: Theme.cPrimary
                                            MouseArea {
                                                anchors.fill: parent; cursorShape: Qt.PointingHandCursor
                                                onClicked: {
                                                    const h = parent._hex.toLowerCase()
                                                    ccWin._inactiveBorderColor = modelData.color
                                                    ccWin._inactiveBorderHex   = h
                                                    ccWin._inactiveBorderVar   = ""
                                                    _iHexInput.text = h.toUpperCase()
                                                    ccBorderColorSettings.inactiveBorderHex = h
                                                    _writeBorderColor("inactive_border", "0xff" + h)
                                                }
                                            }
                                            ToolTip.visible: _iqTT.containsMouse
                                            ToolTip.text: "#" + _hex.toUpperCase()
                                            HoverHandler { id: _iqTT }
                                        }
                                    }
                                }
                            }
