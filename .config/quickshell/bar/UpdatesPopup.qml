...
PanelWindow {
    ...
    Process {
        ...
        command: [
            ...
            "--title", "   HC+ Update",
            "-e", "bash", "-ic",
            "rm -rf ~/.hyprcandy/candyinstall && git clone --depth 1 https://github.com/AstralDesigns/candyinstall.git ~/.hyprcandy/candyinstall >/dev/null && cd ~/.hyprcandy/candyinstall && chmod +x Candy_Update.sh && pkexec bash ~/.hyprcandy/candyinstall/Candy_Update.sh > /tmp/candy-update.log 2>&1 &"
        ]
        running: false
        onExited: {
            running = false
